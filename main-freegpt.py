...
def web_main():
    ...
    def receive_settings_from_frontend():
        global SECURITY_OPTION, ImageGeneration, Weather, _AI, Personality
        data = request.get_json()
        
        SECURITY_OPTION = data.get('password')
    ...
    def receive_data_from_frontend():
        global Wikipidia, Wolfarm, ImageGeneration, Weather, _AI, Personality, YTdriver
        ...
        if "sir" in message and enableteachers == True:
            message = message.replace("sir", "")
            message = message.replace(" ", "")
            message = message.lower()
            path = "interface/" + message + ".jpg"
            msg1 = "Here is the image:"
            msg2 = "An honerable professor of Chandpur govt. College"
                
        
        elif ("show me" in message or "picture of" in message):
